...
use super::{
    ch_queue::{parallel_queue::ParallelQueue, queue::CHQueue},
    shortcut::Shortcut,
};
...
impl ParallelContractor {
    pub fn new(graph: &Graph, priority_functions: &str) -> Self {
        ...
        let queue = ParallelQueue::new(&graph, 3);
    ...
    pub fn contract_single_vertex(&mut self) -> Vec<Shortcut> {
        ...
        while let Some(v) = self.queue.pop(&self.graph) {
            let neighbors: HashSet<_> = v
                .par_iter()
                .map(|&(vertex, _)| self.graph.open_neighborhood(vertex, 1))
                .flatten()
                .collect();
            ...
            self.queue.update(&neighbors, &self.graph);
