...
// use crate::{
//     graphs::{
//         reversible_graph::ReversibleGraph, Distance, Edge, Graph,
// TaillessEdge, Vertex,         WeightedEdge,
//     },
...
//     new_and_updated_edges: &HashMap<Vertex, (Vec<TaillessEdge>,
// Vec<TaillessEdge>)>,     vertex: Vertex,
// ) -> i32 {
...
//     let mut queue: BinaryHeap<Reverse<(i32, Vertex)>> =
// (0..graph.out_graph().number_of_vertices())         .into_par_iter()
//         .progress()
//         .map(|vertex| {
//             let new_and_updated_edges =
//                 par_simulate_contraction_distance_heuristic(&graph,
// distance_heuristic, vertex);             let edge_difference =
...
//     writeln!(
//         writer,
//         "create edges,to update,update edge map,disconnect,insert and
// update,update queue"     )
//     .unwrap();
...
//         let new_and_updated_edges =
//             par_simulate_contraction_distance_heuristic(&graph,
// distance_heuristic, vertex);         let p0 = start.elapsed().as_secs_f64();
//
//         let start = Instant::now();
...
//
//         writeln!(writer, "{},{},{},{},{},{}", p0, p1, p2, p3, p4,
// p5).unwrap();         writer.flush().unwrap();
//     }
//     pb.finish();
//
...
//
// fn update_edges_map(
//     new_and_updated_edges: &HashMap<u32, (Vec<TaillessEdge>,
// Vec<TaillessEdge>)>,     edges: &mut HashMap<(Vertex, Vertex), Distance>,
// ) {
...
//
//                 // (tail -> vertex -> head) cant be a shortest path if (tail
// == head)                 if tail == head {
//                     continue;
//                 }
//
...
//
//                 // if there is already an edge (tail -> head) but
// shortcut_distance is less than                 // the current distance,
// update edge weight                 if let Some(current_distance) =
// graph.get_weight(&edge.remove_weight()) {                     if
// shortcut_distance < current_distance {                         
// updated_edges.push(edge.remove_tail());                     }
//                     continue;
//                 }
//
//                 // if shortcut_distance <= upper_bound_distance {
//                 if distance_heuristic.is_less_or_equal_upper_bound(tail,
// head, shortcut_distance) {                     
...
//
//         let shortcut_distance = in_edge.weight + out_edge.weight;
//         let lower_bound = distance_heuristic.upper_bound(in_edge.head,
// out_edge.head);
//
