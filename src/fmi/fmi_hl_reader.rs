...
fn hl_from_reader<D: Distance + FromStr, R: Read>(reader: R) -> Option<HubLabeling<D>> {
...
pub fn read_fmi_hl<D: Distance + FromStr>(file: &std::path::Path) -> Option<HubLabeling<D>> {
    let reader = BufReader::new(File::open(file).unwrap());
    hl_from_reader(reader)
}
