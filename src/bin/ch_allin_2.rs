...
use faster_paths::{
    ...
    utility::{benchmark_and_test_distance, generate_test_cases, get_progressbar},
};
...
fn main() {
    ...
    let level_to_vertex_org: Vec<Vertex> = serde_json::from_reader(reader).unwrap();

    let reader = BufReader::new(File::open(args.simple_graph_hl).unwrap());
    ...
    let heuristic = CombinedHeuristic {
        heuristics: vec![
            Box::new(hub_graph), /* xx
                                  * Box::new(landmarks), // xx */
        ],
    };
    ...
    let pb = get_progressbar("contracting", level_to_vertex_org.len() as u64);
    for &vertex in level_to_vertex_org.iter() {
        pb.inc(1);
        level_to_vertex.push(vertex);

        let this_edges = contract(&mut graph, &heuristic, vertex);
...
pub struct CombinedHeuristic {
    pub heuristics: Vec<Box<dyn DistanceHeuristic>>,
}

impl DistanceHeuristic for CombinedHeuristic {
    fn lower_bound(&self, _source: Vertex, _target: Vertex) -> Distance {
        self.heuristics
            .iter()
            .map(|h| h.lower_bound(_source, _target))
            .max()
            .unwrap_or(0)
    }
    ...
    fn upper_bound(&self, _source: Vertex, _target: Vertex) -> Distance {
        self.heuristics
            .iter()
            .map(|h| h.upper_bound(_source, _target))
            .min()
